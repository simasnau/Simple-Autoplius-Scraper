    ...
    try:
        while elementExistsByClass('next'):
            ...
            for post in posts:
                ...
                with lock:
                    # Gets mileage, it may be in different indexes in stats array.
                    if len(stats) > 3 and 'km' in stats[3].text:
                        rida = int(
                            (stats[3].text.strip('km')).replace(" ", ""))
                        datos.append(data)

                        ridos.append(rida)
                    elif len(stats) > 4 and 'km' in stats[4].text:
                        rida = int(
                            (stats[4].text.strip('km')).replace(" ", ""))
                        datos.append(data)
